...
static short vsDLink, errCSum=0, errReset=0, errIndex=0, errDone=0;
...
BOOLEAN DataLinkInit(int port, int baud, BYTE node)
  {
    ...
    vsDLink = VirtualGetNew(DataLinkWINDOW, &height, &width, &attr);
    dlink.port = port;
    dlink.node = node;
...
short DataLinkReceive(char *data)
  {
    ...
    if (dlink.buff[dlink.len-1] != DataLinkCheckSum(dlink.len-1, dlink.buff)) {
      VirtualPrint(vsDLink, 0, 1, GIANT, BWHT, "C");
      VirtualPrint(vsDLink, 0, 6, GIANT, BWHT, "Check Sum (%5d)", ++errCSum);
      return (DLINK_ERROR);
      }
    if (dlink.buff[PACK_DATA]==ESC && dlink.buff[PACK_DATA+1]==DLINK_RESET) {
      ...
      dlink.oldIndex = DLINK_INDEX_BAD;
      dlink.curIndex = DLINK_INDEX_INIT;
      VirtualPrint(vsDLink, 0, 1, GIANT, BWHT, "R");
      VirtualPrint(vsDLink, 0, 28, GIANT, BWHT, "Reset (%5d)", ++errReset);
      return (DLINK_ERROR);
      }
    if (dlink.buff[PACK_IDX] == dlink.oldIndex) {
      DataLinkTransmitError(ERR_DONE);
      VirtualPrint(vsDLink, 0, 1, GIANT, BWHT, "D");
      VirtualPrint(vsDLink, 0, 46, GIANT, BWHT, "Done (%5d)", ++errDone);
      return (DLINK_ERROR);
      }
    if (dlink.buff[PACK_IDX] != dlink.curIndex) {
      DataLinkTransmitError(ERR_INDEX);
      VirtualPrint(vsDLink, 0, 1, GIANT, BWHT, "I");
      VirtualPrint(vsDLink, 0, 63, GIANT, BWHT, "Index (%5d)", ++errIndex);
      return (DLINK_ERROR);
      }
